...
class XianyuLive:
    def __init__(self, cookies_str):
        ...
        self.delivery_config_path = os.getenv("DELIVERY_CONFIG_PATH", "data/delivery_items.json")
    ...
    def load_delivery_config(self):
        ...
        try:
            if not os.path.exists(self.delivery_config_path):
                logger.warning(f"未找到自动发货配置文件: {self.delivery_config_path}")
                return {}

            with open(self.delivery_config_path, "r", encoding="utf-8") as f:
                config = json.load(f)

            items = config.get("items", config)
            if not isinstance(items, dict):
                logger.warning("自动发货配置格式错误：items 必须是对象")
                return {}
            return items
        except Exception as e:
            logger.error(f"读取自动发货配置失败: {e}")
            return {}

    def build_delivery_message(self, item_id):
        """根据商品ID生成发货文案。"""
        delivery_items = self.load_delivery_config()
        item_config = delivery_items.get(str(item_id))

        if not item_config:
            return None

        if isinstance(item_config, str):
            return item_config.strip()

        if not isinstance(item_config, dict) or item_config.get("enabled", True) is False:
            return None

        name = item_config.get("name", "资料")
        link = item_config.get("link", "").strip()
        code = item_config.get("code", "").strip()
        note = item_config.get("note", "").strip()
        message = item_config.get("message", "").strip()

        if message:
            return message

        if not link:
            logger.warning(f"商品 {item_id} 已配置自动发货，但缺少 link")
            return None
    ...
    def _collect_strings(self, value):
        ...
        if isinstance(value, str):
            ...
            for item in value.values():
                strings.extend(self._collect_strings(item))
        elif isinstance(value, list):
            for item in value:
                strings.extend(self._collect_strings(item))
        return strings

    def _find_value_by_keys(self, value, keys):
        """递归查找指定字段名的值。"""
        if isinstance(value, dict):
            for key, item in value.items():
                if key in keys and item:
                    return str(item).split("@")[0]
                found = self._find_value_by_keys(item, keys)
                if found:
                    return found
        elif isinstance(value, list):
            for item in value:
    ...
    def extract_order_info(self, message):
        """从订单通知中提取买家ID、商品ID和会话ID。"""
        strings = self._collect_strings(message)
        joined = "\n".join(strings)

        user_id = None
        raw_user = message.get("1") if isinstance(message, dict) else None
        if isinstance(raw_user, str) and "@goofish" in raw_user:
            user_id = raw_user.split("@")[0]

        item_id = None
        item_match = re.search(r"(?:itemId=|item_id[=:]|itemId[=:])(\d+)", joined)
        if item_match:
            item_id = item_match.group(1)
        else:
            item_id = self._find_value_by_keys(message, {"itemId", "item_id", "itemID"})

        chat_id = self._find_value_by_keys(message, {"chatId", "chat_id", "cid", "conversationId"})

        if user_id and item_id:
            mapped_chat_id = self.context_manager.get_chat_id_for_order(user_id, item_id)
            if mapped_chat_id:
                chat_id = mapped_chat_id

        return user_id, item_id, chat_id

    async def handle_paid_order(self, websocket, message):
        """处理付款后自动发货。"""
        user_id, item_id, chat_id = self.extract_order_info(message)

        if not user_id:
            logger.warning(f"检测到待发货订单，但无法提取买家ID: {message}")
            return

        if not item_id:
            lookup_user_id, lookup_item_id = self.context_manager.get_latest_order_context_by_chat(user_id)
            if lookup_user_id and lookup_item_id:
                logger.info(f"订单通知缺少商品ID，已通过会话 {user_id} 回查到买家 {lookup_user_id}、商品 {lookup_item_id}")
                chat_id = user_id
        ...
        if not item_id:
            logger.warning(f"交易成功 {user_url} 等待卖家发货，但无法提取商品ID，跳过自动发货")
            return

        await self.send_delivery_for_order(websocket, user_id, item_id, chat_id)

    async def send_delivery_for_order(self, websocket, user_id, item_id, chat_id, delivery_key=None):
        """根据明确的买家、商品和会话信息发送自动发货内容。"""
        user_url = f"https://www.goofish.com/personal?userId={user_id}"
        reminder_key = delivery_key or f"{user_id}:{item_id}:{int(time.time())}"
        self.context_manager.save_restock_reminder(reminder_key, item_id, user_id, chat_id)
        logger.warning(f"【补上架提醒】商品 {item_id} 已售出，请手动重新上架或补库存。买家: {user_url}")

        delivery_message = self.build_delivery_message(item_id)
        if not delivery_message:
            logger.warning(f"交易成功 {user_url} 商品 {item_id} 未配置自动发货内容，跳过")
            return

        if not chat_id:
            logger.warning(f"商品 {item_id} 无法定位会话ID，跳过自动发货。买家: {user_url}")
            return

        delivery_key = delivery_key or f"{user_id}:{item_id}"
        if self.context_manager.has_delivery_record(delivery_key):
            logger.info(f"发货记录 {delivery_key} 已存在，跳过重复发送")
            return

        await self.send_msg(websocket, chat_id, user_id, delivery_message)
        self.context_manager.save_delivery_record(delivery_key, user_id, item_id, chat_id, delivery_message)
        self.context_manager.add_message_by_chat(chat_id, self.myid, item_id, "assistant", delivery_message)
        logger.info(f"已自动发货给买家 {user_id}，商品 {item_id}")

    def is_paid_delivery_notice(self, message):
        """识别聊天流里的已付款待发货提示。"""
        if not message or not isinstance(message, str):
            return False
        return "已付款" in message and "等待" in message and "发货" in message
    ...
    def is_platform_notice(self, message):
        """检查是否为闲鱼平台提示，避免机器人回复官方风控/防诈提醒。"""
        if not message or not isinstance(message, str):
            return False

        platform_notice_keywords = [
            "闲鱼客服不会",
            "缴纳保证金",
            "开通服务保障",
            "请勿轻信",
        ...
        return any(keyword in message for keyword in platform_notice_keywords)
        ...
        try:
            ...
            try:
                ...
                if message['3']['redReminder'] == '等待买家付款':
                    ...
                    await self.handle_paid_order(websocket, message)
                    return
            ...
            if send_user_id != self.myid:
                self.context_manager.save_chat_lookup(send_user_id, item_id, chat_id)
            ...
            if self.is_manual_mode(chat_id):
                ...
                return
            # 闲鱼有时把已付款待发货作为聊天系统消息推送，这里直接触发自动发货
            if self.is_paid_delivery_notice(send_message):
                logger.info(f"检测到付款发货提示，准备自动发货 (会话: {chat_id}, 商品: {item_id})")
                delivery_key = f"{send_user_id}:{item_id}:{chat_id}:{create_time}"
                await self.send_delivery_for_order(websocket, send_user_id, item_id, chat_id, delivery_key=delivery_key)
                return
            # 检查是否为系统消息
            if self.is_bracket_system_message(send_message) or self.is_platform_notice(send_message):
            ...
            if not item_info:
                ...
                if 'data' in api_result and 'itemDO' in api_result['data']:
                    ...
                    logger.warning(f"获取商品信息失败，使用简化商品信息继续回复: {api_result}")
                    item_info = {
                        "title": f"商品 {item_id}",
                        "desc": "这是电子资料类商品。商品详情暂时无法从闲鱼接口获取时，请基于买家问题进行简洁、保守的客服回复。买家问是否还有、在不在、能否发货时，默认回复有的/在的，并引导拍下后发送资料领取方式；不要说没货。",
                        "soldPrice": 0,
                        "quantity": 999,
                        "skuList": [],
                    }
    ...
    async def main(self):
        while True:
            try:
                ...
                async with websockets.connect(self.base_url, extra_headers=headers, ssl=ssl_context) as websocket:
                    self.ws = websocket
